            ...
            <tbody>
                @foreach ($estudiantes as $estudiante)
                    <tr class="hover:bg-blue-100">
                        ...
                        <td class="py-2 px-4 border-b border-gray-300">
                            <a href="{{ route('estudiantes.edit', $estudiante->id) }}" class="inline-block bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded">
                                Editar
                            </a>
                        </td>
                        <td class="py-2 px-4 border-b border-gray-300">
                            <form action="{{ route('estudiantes.destroy', $estudiante->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('¿Estás seguro de eliminar a {{ $estudiante->nombres }} {{ $estudiante->pri_ape }}?');">
                                @csrf
                                @method('DELETE')
